...
class ArticleController extends AbstractController
{
    ...
    public function adminComments(?Article $article,): Response|RedirectResponse
    {
        ...
        if (!$comments) {
            $this->addFlash('error', 'Aucuns commentaires trouvé pour cet article');

            return $this->redirectToRoute('admin');
        }

        return $this->render('Backend/Comments/index.html.twig', [
            'comments' => $comments,
            'article' => $article,
        ]);
    }
    ...
    public function switchVisibilityComment(?Comment $comment): Response
    {
        if (!$comment instanceof Comment) {
            return new Response('Commentaire inexistant', 404);
        }
    ...
    {
        if (!$comment instanceof Comment) {
            $this->addFlash('error', 'Commentaire inexistant');
